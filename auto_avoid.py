        ...
        if result < 5:
            ...
            if move==True:
                ...
                if point ==0:
                    ...
                    yaw = 180+math.degrees(math.atan((y - y1) / (x - x1)))
                    client.moveToPositionAsync(x1, y1, z, speed, yaw_mode=airsim.YawMode(False, yaw))
                elif point==1:
                    print("point 2")
                    x1=  -8.556684494018555
                    y1= -105.05833435058594
                    yaw = math.degrees(math.atan((y - y1) / (x - x1)))
                    client.moveToPositionAsync(x1, y1, z, speed ,yaw_mode=airsim.YawMode(False, yaw))
                elif point==2:
                    print("point 3")
